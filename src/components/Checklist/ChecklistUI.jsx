...
// Desativa o alerta do Vite Fast Refresh para esta constante
...
export const FormSection = ({ title, icon, children }) => {
  const IconComponent = icon; // Resolve o erro de "Icon never used"
...
export const ToggleCard = ({ label, checked, onChange, icon, disabled }) => {
  const IconComponent = icon; // Resolve o erro de "Icon never used"
  ...
  return (
    ...
    >
      ...
      >
        {IconComponent ? <IconComponent size={14} /> : <Check size={14} />}
      </div>
      <span
